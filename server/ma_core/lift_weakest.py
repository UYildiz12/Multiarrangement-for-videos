...
# --- Compatibility helpers for the server API/tests ---
def get_min_evidence(W: np.ndarray) -> float:
    """Return the minimum off-diagonal evidence value."""
    n = W.shape[0]
    if n < 2:
        return 0.0
    iu = np.triu_indices(n, 1)
    return float(np.min(W[iu]))


def check_stopping_criterion(W: np.ndarray, threshold: float) -> bool:
    """Check stopping based on minimum evidence threshold.

    Matches the adaptive logic in the library (compare min(W) directly).
    """
    if W.size == 0:
        return False
    min_ev = get_min_evidence(W)
    return min_ev >= float(threshold)
